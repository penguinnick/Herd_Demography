---
title: Evaluating Livestock Culling Practices using Population Projection Models in
  Neolithic Dalmatia
...
#   "Figures/Fig1_TheoreticalCullingStrategies_v2.jpg",
#   dpi = 800,
#   width = 6,
#   height = 4
# )
...
ages.list = lapply(wear.df.list, function(a) {
  # adjusts counts for each age group where multiple age groups are listed and returns frequency density
...
# new function for plotting histograms with unequal binwidths (classic mortality profile)
source("./R/mortality_histogram_gg.R")
...
# ggsave(
#   plot = Fig2,
#   filename = "Figures/Fig2_ArchaeologicalMortProfiles_v2.jpg",
#   dpi = 800,
#   width = 8,
#   height = 4
# )
...
```{r, mortality profiles with histograms, include=FALSE, eval=TRUE}
source("./R/mortality_histogram_gg_ci.R")

mort.plots <-  lapply(wear.df.list, function(a){mortality.histogram.gg.CI(a, CI = TRUE, iter = 1000, ci = 95, probability.correction = TRUE)})
mort.plots = c(mort.plots[3], mort.plots[4], mort.plots[1], mort.plots[2], mort.plots[5])
# Fig2
Fig2v3 = cowplot::plot_grid(plotlist = mort.plots, labels = names(mort.plots), 
                          label_size = 10, 
                          vjust = 1.5, hjust = c(-1.5, -1.5, -0.75, -0.75, -0.75))
...
# ggsave(
#   plot = Fig2v3,
#   filename = "Figures/Fig2_ArchaeologicalMortProfiles_v3.jpg",
#   dpi = 800,
#   width = 8,
#   height = 4
# )
...
```{r, plotFig2, echo=FALSE, fig.dim=c(8 , 4), fig.cap="Figure 2. Archaeological mortality profiles."}
Fig2v3
```
### Supplementary Table S4.1: Mandible sample information
```{r, TableS4, echo=FALSE, include=TRUE}
TableS4.1 <- wear.df %>% 
  filter(Payne.Group != "") %>%
  # mutate(N=n()) %>% 
...
#-- create output word doc
# save_as_docx(TableS4.1, path = "tables/TableS4-1_mandible_counts.docx", pr_section = sect_properties)
```
...
```{r, Load-reproboot-results, include=FALSE}
repro.boot.df <-  read.table("output/reproboot.csv", header = T, sep = ",")
```

```{r, facet_manual_helper_function, echo=FALSE, include=FALSE}
facet_factor_fun <- function(dat){
  dat %>% mutate(Author = case_when(
      ...
      .default = "Neolithic" )) %>%
  mutate(
    Author = factor(Author, levels = c( "Vigne and Helmer (2007)", "Payne (1973)", "Redding (1981)",
                                        "Neolithic", "No Offtake" )),
    strategy = factor(strategy, levels = c(names(offtake.mortality)[6:10], 
...
Fig3 = repro.boot.df %>% facet_factor_fun() %>% 
  ...
  # ggplot(aes(y = strategy, x = Lambda, color = Taxon)) +
...
# ggsave(
#   Fig3,
#   filename = "./Figures/Fig3_lambda-boot_v2.jpg",
#   dpi = 800,
#   width = 6,
#   height = 4
# )
...
repro.boot.df %>% facet_factor_fun() %>% 
  select(Taxon, strategy, Lambda, `Proportion Female`, `Proportion Male`, Author) %>%
  pivot_longer(cols = c(4, 5)) %>%
  mutate(
    Lambda = round(Lambda, 3),
    Sex = case_when(name == "Proportion Female" ~ "Female", name == "Proportion Male" ~ "Male"),
  ) %>%
  mutate(Strat = paste0(strategy, " (λ=", Lambda, ")")) %>%
  ggplot() +
  geom_col(aes(x = Taxon, y = value, fill = Sex), position = "dodge") +
  ylab("Proportion") +
...
Fig4
# ggsave("Figures/Fig4_herd_structure_v2.jpg",
#        Fig4,
#        dpi = 300,
#        width = 6,
#        height = 4)
...
# 
# ggsave(
#   "Figures/Fig5_projections_v2.jpg",
#   plot =  Fig5,
#   dpi = 300,
#   width = 6,
#   height = 4
...
Run replicated simulations save time by loading results from output folder.
```{r, run-replicates, echo=TRUE, cache=TRUE, eval=FALSE}
...
```{r, load-replicates, echo=TRUE, eval=TRUE}
load(file = "output/stochastic_sim_res.RData")
```


```{r, summarize-replicates, echo=TRUE}
...
```{r, Figure6, warning=FALSE, echo=FALSE, fig.dim=c(6,4), fig.cap="Figure. S3.7. Plots showing results of replicated projections of change in sheep and goat herd sizes through time under the various survivorship profiles used." }
...
# 
# ggsave(
#   "Figures/Fig6_stochastic_rep_v2.jpg",
#   plot =  Fig6,
#   dpi = 300,
#   width = 6,
#   height = 4
...
plot.projection(new.pop.res.df, offtake.list = offtake.mortality)
```
...
Fig7 = all.res.df %>%  
  ...
  reframe(time, strategy, offtake, Taxon, pop, tot.pop = sum(pop)) %>% 
  unique() %>%
  facet_factor_fun() %>%
...
```{r, Fig9, warning=FALSE, echo=FALSE, eval=TRUE,  fig.dim=c(6,4), fig.cap="Figure S3.11. Showing multiplication rates of unadjusted and adjusted offtake rates for goats"}
plot.m("goat")
```

```{r, Fig10, warning=FALSE, echo=FALSE, eval=TRUE,  fig.dim=c(6,4), fig.cap="Figure S3.12. Showing multiplication rates of unadjusted and adjusted offtake rates for sheep"}
plot.m("sheep")
```
...
Here we implement a Sobol' sensitivity test to evaluate the sensitivity of the optimization algorithm to different herd growth rate thresholds and female offtake rates. We focus on the effects of these parameters on mean herd size over the 200-year projection, as this is a key output of interest for understanding the long-term implications of modifying culling rates. Variance is also of interest, as it can indicate the stability of the herd under different parameter values.

The size of the parameter space will have implications for computational load. Adjusting `np` and `nboot.sens` values increases parameter space. Full sensitivity analysis can be executed on a HPC using the Rscript "XX_SENSITIVITY.R"

```{r, sobol-parap-space, include=TRUE, echo=TRUE, eval=FALSE}
set.seed(123)
#-- 500 samples (2x)
np=500
...
The parameter values are passed to the `adjust.offtake()` function, which applies the optimization algorithm to adjust culling rates based on the specified thresholds and initial herd size. The resulting adjusted parameters are then used to project the herd dynamics using `projectHerd2()`. The outputs of these projections are summarized with `pop_summary2()` and mean and standard deviation of population size is calculated. 

The `sensitivity_wrapper()` function in this chunk all of this using the parameter space defined above. Another helper function `extract_indices()` is defined to extract the Sobol' sensitivity indices from the results of the sensitivity analysis. 

```{r, Sobol-sensitivity, echo=TRUE, include=TRUE, eval=TRUE}
...
To limit computation time we focus only on the Meat, Milk, and Wool models [@Payne1973]. Parallel processing is necessary for large parameter spaces, and the `parallel` package is used to run the sensitivity analysis across multiple cores. The `parLapply()` function is used to apply the `sensitivity_wrapper()` function across all sets of parameters for the Meat, Milk, and Wool models for sheep and goats. The results are then processed to extract the Sobol' sensitivity indices for each output variable with respect to the input parameters. Calculations were performed on UBELIX (https://www.id.unibe.ch/hpc), the HPC cluster at the University of Bern. The code used is found in the Rscript "XX_SENSITIVITY.R".

```{r, parallel-sensitivity, include=TRUE, echo=TRUE, eval=FALSE}
...
## Results
The First-order indices of the Sobol sensitivity analysis indicate how much variance in the output results is due to each parameter in isolation. Here we show only the First-order indices of Female offtake rate. 
```{r, show-sensitivity-results, include=TRUE, eval=TRUE, echo=FALSE, caption="Table 7. Main effects of Female Offtake Rate on variance in mean herd size and inter-annual variation (sd) for the Meat, Milk, and Wool strategies for sheep and goats."}
T7 = sobol_indices %>% 
...
```{r, plot code Fig11 total effect indices, include=FALSE, echo=FALSE, eval=TRUE, warning=FALSE}
param_labels <- c(
...
Comparing the main effects of the parameters across the different strategies and taxa can provide insights into which parameters are most influential in determining the mean herd size and its variability. The following figure shows the main effects of all parameters on mean herd size for the Meat, Milk, and Wool strategies for sheep and goats.

```{r Fig11, include=TRUE, echo=FALSE, fig.dim=c(6,4), fig.cap="Figure 11. Main effects of parameters on mean herd size for the Meat, Milk, and Wool strategies for sheep and goats."}
Fig11 

```


```{r, plot code Fig12 total effect indices, include=FALSE, eval=TRUE, echo=FALSE, warning=FALSE}
Fig12 <- sobol_indices %>%
  facet_factor_fun() %>% 
  mutate(res = factor(res, levels = c("size", "sd"))) %>% 
  # filter(param != "female.offtake.range") %>% 
...
This figure shows the Total effects, which include both the main effects and the interactions between parameters, of all parameters on mean herd size for the Meat, Milk, and Wool strategies for sheep and goats. Comparing the total effects across the different strategies and taxa can provide insights into which parameters are most influential in determining the mean herd size when considering both their individual effects and their interactions with other parameters.

```{r, Fig12, include=TRUE, echo=FALSE, fig.dim=c(6,4), fig.cap="Figure 12. Total effects of parameters on mean herd size for the Meat, Milk, and Wool strategies for sheep and goats."}
Fig12
```
