...
```{r, load-sensitivity-results, include=FALSE, eval=TRUE }
#-- load results
...
T7
save_as_docx(T6, path = "./tables/Table7_Sensitivity_Main_Effects.docx", pr_section = sect_properties)

```


```{r, Fig11 - plot total effect indices}
param_labels <- c(
...
```{r, Fig12 plot total effect indices}
Fig12 <- sobol_indices %>%
  facet_factor_fun() %>% 
  mutate(res = factor(res, levels = c("size", "sd"))) %>% 
  # filter(param != "female.offtake.range") %>% 
