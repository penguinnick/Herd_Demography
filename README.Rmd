...
`HerdDynamics` includes survivorship probabilities associated with Redding's [-@Redding1981] **Energy** and **Security**,  Payne's [-@Payne1973] **Meat**, **Milk**, and **Wool** strategies, and Vigne and Helmer's [-@Vigne2007] Meat and Milk types A and B (**MeatA**, **MeatB**, **MilkA**, **MilkB**) and **Fleece** strategies. Marom and Bar-Oz [-@Marom2009] provide survivorship probabilities that were standardized across nine age classes. These data are stored as a list in the HerdDynamics package as `offtake_models`. 

The offtake rates given here are generated from zooarchaeological research which does not account for sex-based differences in culling practices.  But these offtake rates are often interpreted on the level of the herd regardless of sex. Here they are used to model the offtake rates of males. These rates are reduced for females in a later step. 
...
T2 = cbind.data.frame(c("", Payne_ages$ageClasses) , do.call( cbind.data.frame, args = c( offtake_models) ) )
...
```{r, Table-3-site-survivorship, echo=FALSE}
#-- Create Table of survival probabilities from culling data allocated to distribution of ages used in model

T3 = cbind.data.frame( c("", Payne_ages$ageClasses) , do.call( cbind.data.frame, args = c( culling.profiles )) )
...
HerdDynamics::culling_multiplot(c(offtake_models, culling.profiles))
...
set.seed(388)
# generate_prolificacy_rates(meanPro = NetPro$mn.pro[1], sdPro = NetPro$sd.pro[1], n = 6)
# 
# sapply(c("Goat", "Sheep")), function(x) {
#   param.dat$Prolificacy[param.dat$ParturitionRate>0 & param.dat$Taxon==x] = generate_prolificacy_rates(meanPro = NetPro$mn.pro[1], sdPro = NetPro$sd.pro[1], n = 6)
#   param.dat$ParturitionRate[param.dat$ParturitionRate>0 & param.dat$Taxon==x] = Part.rate(NetPro$mn.pro[1], parturition.Interval)
# })
# 
#-- generate Prolificacy rates for Table 2 using function above
param.dat$Prolificacy[param.dat$ParturitionRate>0 & param.dat$Taxon=="Goat"] = generate_prolificacy_rates(meanPro = NetPro$mn.pro[1], sdPro = NetPro$sd.pro[1], n = 6)
param.dat$Prolificacy[param.dat$ParturitionRate>0 & param.dat$Taxon=="Sheep"] = generate_prolificacy_rates(meanPro = NetPro$mn.pro[2], sdPro = NetPro$sd.pro[2], n = 6)

param.dat$ParturitionRate[param.dat$ParturitionRate>0 & param.dat$Taxon=="Goat"] = ARR(NetPro$mn.pro[1], parturition.Interval)

param.dat$ParturitionRate[param.dat$ParturitionRate>0 & param.dat$Taxon=="Sheep"] = ARR(NetPro$mn.pro[2], parturition.Interval)
...
# # getwd()
t1 %>% set_caption("Table 1. Parameters used to construct the Lefkovitch population projection matrix for sheep and goats. Parturition and prolificacy rates one possible outcome of the sampling procedure.")
# #-- create output word doc
...
Using the functions defined above ('vary.fert.mort()' and 'make.listpar()'), a stochastic environment is created for each offtake strategy. The function `make.listpar()` is run with the parameters defined above. The number of cycles is set to `200` and the initial population size is set to `150`.
...
repro.boot.df %>% filter(strategy == "Baseline")
```
...
#-- labeller function
strip_labeller <- function(var, val){
  return(strip.labs[val])
}
```
...
```{r Figure4, echo=-3, fig.dim=c(6,4), include=FALSE, eval=FALSE}
#-- This chunk creates Figure 4, showing herd structure for each strategy
...
Fig4
ggsave("Figures/Fig4_herd_structure.jpg", Fig4, width = 6, height = 4)
```
...
```{r, Fig5, echo=FALSE, warning=FALSE, fig.dim=c(6,4), fig.cap="Figure. 5. Plots showing the change in sheep and goat herd sizes through time under the various survivorship profiles used. " }
#-- plot
Fig5 = tot.pop.df %>% 
...
ggsave( "Figures/Fig5_projections.jpg", plot =  Fig5 , width = 6, height = 4)
...
```{r Figure7, warning=FALSE, echo=FALSE, fig.dim=c(6,4), warning=FALSE,  fig.cap="Figure. 7. Plots showing results of replicated projections of change in sheep and goat herd sizes through time under the various survivorship profiles used." }
#-- plot results
Fig7 = sto.res.df %>%
  mutate(Taxon = taxon) %>%
...
# ggsave( "Figures/Fig7_stochastic_rep.jpg", plot =  Fig7 , width = 6, height = 4)
...
Fig8
ggsave( "Figures/Fig8_reprojections.jpg", plot =  Fig8 , width = 6, height = 4)
...
This chunk uses a modified version of `mmage::fprod()` function from the `mmage` package [@mmage] to calculate mean herd size as shown in Figure 9.
```{r, xmean, echo=-c(7:10), fig.dim=c(6,4), fig.cap="Figure 9. Mean herd size after optimized culling rates applied"}
...
Fig9
# ggsave( "Figures/Fig9_mean_herd_size.jpg", plot =  Fig7 , width = 6, height = 4)
...
#-- labeller function
sheep_labeller <- function(var, val){
  return(sheep.labs[val])
}
...
#-- labeller function
goat_labeller <- function(var, val){
  return(goat.labs[val])
}
...
# Fig8
ggsave( "Figures/Fig10_Lambda_goat.jpg", plot =  plot.m("goat") , width = 6, height = 4)
# Fig9
ggsave( "Figures/Fig11_Lambda_sheep.jpg", plot =  plot.m("sheep") , width = 6, height = 4)
```

### Levene's Test for Equality of Variance
This section performs a Levene's Test for Equality of variances between the herd multiplication rate after offtake was optimized and the predicted $\lambda$ value (i.e., unadjusted rates).
