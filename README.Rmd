...
```{r, theoretical-survivorship-table,  include=FALSE} 
...
Age-at-death data for sheep and goat mandibles collected from **Benkovac-Barice**, **Graduša-Lokve at Islam Grčki**, **Smilčić**, and **Zemunik Donji** are used to create survival probabilities for each age class [@Triozzi2024]. 
...
# }
# 
# strip_labeller <- function(labels){
#   strip.labeller$lambda.lab
# } 
...
Fig5 = tot.pop.df %>% 
  ...
  facet_wrap( ~ strategy, labeller = as_labeller(strip_labels), scales = "free_y" ) + 
  theme_minimal() +
...
Fig7 = sto.res.df %>%
  ...
  theme_minimal() +
  facet_wrap( ~ strategy, scales = "free_y", labeller = as_labeller(strip_labels) ) +
