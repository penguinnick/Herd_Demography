...
## Overview
The population projection model presented here uses the `HerdDynamics` package, which features wrapper functions for the [mmage package](https://gitlab.cirad.fr/selmet/livtools/mmage) [@Lesnoff2024] and customized scripts for comparing different herd culling strategies [@Triozzi2024]. In the `HerdDynamics` package, procedures described in the [mmage reference manual](https://gitlab.cirad.fr/selmet/livtools/mmage/-/blob/master/doc/mmage_v1.7.pdf?ref_type=heads) have been consolidated into wrapper functions such as "build_tcla", and "build.param". 
...
T1ft %>% set_caption("Table S1.2. The theoretical culling strategies with survivorship probabilities standardized by Marom and Bar-Oz (2009).")
...
T2$Age = c(0,ages)
# ageClasses2 = c( "A (0-2m)" , "B (2-6m)" , "C (6-12m)" , "D (1-2y)" , "EF (2-4y)" , "G (4-6y)" , "HI (6-10y)" , "10y+")
# T2 = cbind.data.frame( ageClasses2 , do.call( cbind.data.frame, args = c( culling.profiles )) )
# T2 = cbind.data.frame( ageClasses , do.call( cbind.data.frame, args = c( culling.profiles2 )) )
# T2$Age = c( 0.17, 0.50, 1.00, 2.00, 4.00, 6.00, 10.00, Inf )
...
T2ft = flextable(T2[-1,c(1, 7, 4,5,2,3,6)]) %>%
  ...
  paginate(
    ...
    group = "ageClasses", # keep Sample.ID group across pages
    group_def = "rle"
    ) %>%
  theme_vanilla()

T2ft %>% set_caption("Table S1.3. Survival probabilities for caprines at four Neolithic Sites in Dalmatia, standardized into age groups used by Marom and Bar-Oz (2009).")
...
HerdDynamics::plot_offtake(
  lapply(offtake.models, function(l) 1-(l*0.01)) , ages = as.factor(c(0,ages)), title = "Survivorship curves") +
  scale_x_discrete( labels = ageClasses  ) +
  xlab("Age Class") +
  theme_minimal() +
...
# -- Fig 2
HerdDynamics::plot_offtake(
  lapply(culling.profiles, function(l) 1-(l*0.01)) , ages = as.factor(c(0,ages)), title = "Survivorship curves") +
  scale_x_discrete( labels = ageClasses  ) +
  xlab("Age Class") +
  theme_minimal() +
  theme( axis.text.x = element_text( angle = 45))
